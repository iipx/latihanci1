...
class Model_why_choose extends CI_Model 
{
    ...
    function show() {
        $sql = "SELECT * FROM tbl_why_choose ORDER BY id ASC";
        $query = $this->db->query($sql);
        return $query->result_array();
    }
    ...
    function get_photo()
    {
        $sql = 'SELECT * FROM tbl_why_choose_photo WHERE id=?';
        $query = $this->db->query($sql,array(1));
        return $query->first_row('array');
    }
    function update_why_choose_photo($data) {
        $this->db->where('id',1);
        $this->db->update('tbl_why_choose_photo',$data);
    }
